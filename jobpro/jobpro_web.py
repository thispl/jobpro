...
def new_candidate(full_name, email, password, mobile_no, source, reference_source, media_source , countryData):
...
def apply_jobs(task, candidate):
    ...
    candidate_id = frappe.db.get_value("Candidate",{"mail_id":candidate},"name")
    # if not candidate_id:
    # 	frappe.log_error("Candidate not found for mail_id: " + str(id), "Apply Jobs Error")
    # 	return frappe.as_json({"status": "Error", "message": "Candidate not found"})
    candidate_doc = frappe.get_doc("Candidate",candidate)
    if not candidate_doc:
        frappe.log_error("Candidate not found.... Apply Jobs Error")
        return frappe.as_json({"status": "Error", "message": "Candidate not found"})
    frappe.errprint(task)
    frappe.errprint(id)
    # frappe.errprint(candidate_id)
    frappe.errprint(candidate_doc)
    if candidate_doc.irf:
        task.append("custom_jobpro_candidates", {
            "candidate": candidate_doc.name,
            "given_name": candidate_doc.given_name,
            "status": candidate_doc.pending_for,
        })
        task.save()
        # Send mail to hr
        message = f"""
            ...
            <p></p>
            <p style="color: #05264e;">Candidate {candidate_doc.name} has applied for the Job Post - {task.name}</p>
            <p style="color: #0070cc;">Best regards,<br>JOBPRO Team</p>
        """
        frappe.sendmail(
            ...
            reference_name=candidate_doc.name,
            delayed=False,
        )
        # Send mail to candidate
        message = f"""
            <p style="color: #05264e; font-weight: 700; font-size: 15px;">Dear {candidate_doc.name},</p>
            <p></p>
            <p style="color: #05264e;">You have successfully applied for the job - {task.subject}. Our team will contact you soon</p>
            <p style="color: #0070cc;">Best regards,<br>JOBPRO Team</p>
        """
        frappe.sendmail(
            recipients=[candidate_doc.mail_id],
            subject="Job Application - Reg",
            message=message,
            reference_doctype="Candidate",
            reference_name=candidate_doc.name,
...
#     doc_id = frappe.db.get_value("Candidate",{"mail_id":id},"name")
     
#     frappe.errprint("not found")
#     print(doc_id)
#     frappe.log_error(
#     title="Candidate Debug",
#     message=frappe.as_json(doc_id.as_dict())
# )
#     task = frappe.get_doc("Task", task)
#     applied = []
#     for row in task.custom_jobpro_candidates:
#         if row.candidate == doc_id:
#             applied.append("true")
#     if "true" in applied:
#         return "true"
#     else:
...
def find_applied_jobs(task, id):
    ...
    applied = any(row.candidate == id for row in task.custom_jobpro_candidates)
...
def list_specialization():
    data = []
    specialization = frappe.db.sql("""
                        select name
                          from `tabSpecialization`
                          order by name
                    """, as_dict=True
                    )
...
@frappe.whitelist(allow_guest=True)
def list_currencyCtc():
    data = []
    currencyCtc = frappe.db.sql("""
                        select name
                          from `tabCurrency`
                          where enabled='1'
                          order by name
    ...
    for ctc in currencyCtc:
        data.append(ctc.name)
    return data
...
def get_school_hsc(email):
    data = []
    candidate = frappe.get_doc("Candidate", {"mail_id": email})
    for row in candidate.get("table_28"):
        if row.qualification == "HSC":
            data.append({
...
def is_valid_dob(dob_str, min_age=18, max_age=100):
    dob = datetime.strptime(dob_str, "%Y-%m-%d")
    today = datetime.today()

    if dob > today:
        return "DOB: Future date not allowed"

    age = (today - dob).days // 365
    if age < min_age or age > max_age:
        return "DOB: Age constraints"
...
def update_referpro_profile(email=None, account_number=None, ifsc_code=None, name=None, primary_mobile=None, secondary_mobile=None, dob=None, passport_id=None, aadhaar_id=None, street_name=None, city=None, state=None, postal_code=None, country=None):
    ...
    if dob:
        ...
        if is_valid_dob(dob) != "valid dob":
            return is_valid_dob(dob)
    ...
    if frappe.db.exists("REFERPRO Profile", email):
        ...
        user.full_name = name
        user.primary_mobile = primary_mobile
...
@frappe.whitelist(allow_guest=True)
def get_candidate_id(user_mail):
    frappe.log_error(title='msil',message=user_mail)
    candidate = frappe.db.get_value("Candidate", {"mail_id": user_mail}, "name")
    return candidate
...
def create_purchase_invoice(supplier, item):
    ...
    pi.created_by = frappe.db.get_value("Candidate", supplier, "mail_id")
...
@frappe.whitelist(allow_guest=True)
def test_check():
    get_claim_details(user_id = "amar.p@groupteampro.com", candidate="CD155093")

@frappe.whitelist(allow_guest=True)
def get_claim_details(candidate, user_id):
    data = frappe.db.sql("""
    SELECT 
        pii.item_code AS candidate_id,
        pii.item_name AS name,
        pii.amount AS reward,
        DATE_FORMAT(pi.posting_date, '%%M %%e, %%Y') AS invoice_created_date,
        Date_FORMAT(pe.posting_date, '%%M %%e, %%Y') AS payment_created_date,
        pi.name AS invoice_reference,
        pe.name AS payment_reference,
        CASE
            WHEN pi.docstatus = 0 THEN 'Processing'
            WHEN pi.docstatus = 1 AND pe.name IS NULL THEN 'Approved'
            WHEN pi.docstatus = 1 AND pe.docstatus = 0 THEN 'Payment Pending'
            WHEN pi.docstatus = 1 AND pe.docstatus = 1 THEN 'Paid'
            ELSE 'Unknown'
        END AS status,
        CASE
            WHEN pi.docstatus = 0 THEN 0
            WHEN pi.docstatus = 1 AND pe.name IS NULL THEN 1
            WHEN pi.docstatus = 1 AND pe.docstatus = 0 THEN 2
            WHEN pi.docstatus = 1 AND pe.docstatus = 1 THEN 3
            ELSE 0
        END AS active_step
    ...
    WHERE pi.supplier = %s
      AND pi.created_by = %s
      AND pi.docstatus != 2
""", (candidate, user_id), as_dict=True)
...
@frappe.whitelist(allow_guest=True)
def is_purchase_invoice_created(supplier, item):
    purchase_invoices = frappe.get_all("Purchase Invoice", {"supplier": supplier, "docstatus": ["!=", 2]}, "name")
    if purchase_invoices:
        for pi in purchase_invoices:
            if frappe.db.exists("Purchase Invoice Item", {"parent": pi.name, "item_code": item}):
                return "ok"
            else:
                return "not ok"
    else:
